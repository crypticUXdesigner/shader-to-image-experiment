/**
 * Shared CPU/GPU resources + per-frame encoding for glow/bloom
 * (`pass.glow-bloom.v1` pass plans).
...
export type GlowBloomV1Runtime = {
...
export async function validateGlowBloomV1ShaderModules(
  device: GPUDevice,
  plan: GlowBloomV1Plan
...
export function createGlowBloomV1Runtime(
  device: GPUDevice,
  plan: GlowBloomV1Plan,
...
export function destroyGlowBloomV1Runtime(rt: GlowBloomV1Runtime | null | undefined): void {
  if (!rt) return;
  try {
    if (rt.textures) {
...
function readParamScalar(rt: GlowBloomV1Runtime, slot: number, fallback: number): number {
  if (slot < 0) return fallback;
  const v = rt.paramsData[slot * 4];
  return typeof v === 'number' ? v : fallback;
}

export function encodeGlowBloomV1Frame(
  device: GPUDevice,
  queue: GPUQueue,
  rt: GlowBloomV1Runtime,
  width: number,
  height: number,
  ...
  rt.globalsData[4] = readParamScalar(rt, rt.plan.paramSlots.threshold, 0.7);
  rt.globalsData[5] = readParamScalar(rt, rt.plan.paramSlots.intensity, 1.0);
  rt.globalsData[6] = readParamScalar(rt, rt.plan.paramSlots.radius, 5.0);
  rt.globalsData[7] = readParamScalar(rt, rt.plan.paramSlots.strength, 0.5);
  rt.globalsData[8] = 0.0;
  rt.globalsData[9] = 0.0;
...

  {
    ...
    const pass = encoder.beginRenderPass({
      colorAttachments: [{ view: source.createView(), clearValue: { r: 0, g: 0, b: 0, a: 1 }, loadOp: 'clear', storeOp: 'store' }],
    });
...

  {
    ...
    const pass = encoder.beginRenderPass({
      colorAttachments: [{ view: bright.createView(), clearValue: { r: 0, g: 0, b: 0, a: 1 }, loadOp: 'clear', storeOp: 'store' }],
    });
    pass.setViewport(0, 0, width, height, 0, 1);
...

  {
    ...
    const pass = encoder.beginRenderPass({
      colorAttachments: [{ view: blur.createView(), clearValue: { r: 0, g: 0, b: 0, a: 1 }, loadOp: 'clear', storeOp: 'store' }],
    });
    pass.setViewport(0, 0, width, height, 0, 1);
...

  {
    ...
    const pass = encoder.beginRenderPass({
      colorAttachments: [{ view: presentTargetView, clearValue: { r: 0, g: 0, b: 0, a: 1 }, loadOp: 'clear', storeOp: 'store' }],
    });
    pass.setViewport(0, 0, width, height, 0, 1);
